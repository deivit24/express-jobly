...
describe('GET /companies', async function () {
  ...
  test('Search params are working', async function () {
    ...
    const response = await request(app).get('/companies?search=gon').send({
      _token: TEST_DATA.userToken,
    });
    expect(response.body.companies).toHaveLength(1);
    expect(response.body.companies[0]).toHaveProperty('handle');
  ...
  test('Search params show no results', async function () {
    ...
    const response = await request(app)
      .get('/companies?search=sdfgsdfsdf')
      .send({
        _token: TEST_DATA.userToken,
      });
...
describe('GET /companies/:handle', async function () {
  ...
  test('Responds with a 404 if it cannot find the company in question', async function () {
    const response = await request(app).get(`/companies/yaaasss`).send({
      _token: TEST_DATA.userToken,
    });
...
describe('PATCH /companies/:handle', async function () {
  ...
  test('Responds with a 404 if it cannot find the company in question', async function () {
    ...
    const response = await request(app).patch(`/companies/taco`).send({
      name: 'newTaco',
      _token: TEST_DATA.userToken,
    });
    expect(response.statusCode).toBe(404);
